...
final class WindowStateInspectorTests: XCTestCase {
    ...
    func testOpenAndCloseSettingsSwitchesWindowRoute() {
        let project = Project(
            name: "Repo",
            rootPath: "/tmp/repo",
            canonicalRootPath: "/tmp/repo"
        ...
        XCTAssertEqual(windowState.activeRoute, .workspace)

        windowState.openSettings()
        XCTAssertEqual(windowState.activeRoute, .settings)

        windowState.closeSettings()
        XCTAssertEqual(windowState.activeRoute, .workspace)
    }
}
