...
Use when teams or agents share a durable knowledge layer (e.g., Odyssey blackboard HTTP API, Redis, or JSON files under `~/.odyssey/blackboard/`). Use for facts, tasks, decisions, and cross-session memory—not as a chat log replacement.
