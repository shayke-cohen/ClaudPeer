...
struct ScheduleEditorView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Text("Define what should run, where it should run, and how often ClaudeStudio should trigger it.")
                .font(.callout)
    ...
    private var identitySection: some View {
        ScheduleEditorSectionCard(
            title: "Identity",
            description: "Give the mission a clear name and choose the folder ClaudeStudio should use as the working directory."
        ) {
            ...
            VStack(alignment: .leading, spacing: 18) {
                ...
                if draft.runMode == .reuseConversation, draft.targetKind != .conversation {
                    conversationTargetField(
                        ...
                        detail: "Reuse mode needs an existing thread. ClaudeStudio will append each run into the conversation you pick here."
                    )
                }
            }
        }
    ...
    private var dailyTimeField: some View {
        ScheduleEditorFieldRow(
            title: "Run time",
            detail: "Pick the local time ClaudeStudio should target for each scheduled run."
        ) {
    ...
    private var reliabilitySection: some View {
        ScheduleEditorSectionCard(
            title: "Reliability",
            description: "Control whether the schedule is active and whether ClaudeStudio should try to wake up for it while closed."
        ) {
            VStack(alignment: .leading, spacing: 18) {
                ScheduleEditorFieldRow(
                    title: "Enabled",
                ...
                ScheduleEditorFieldRow(
                    ...
                    detail: "Exports a local launchd helper so ClaudeStudio can wake up and trigger the mission."
                ) {
                    VStack(alignment: .leading, spacing: 8) {
                        Toggle("Run while app is closed", isOn: $draft.runWhenAppClosed)
                            ...
                            .help("Lets ClaudeStudio install a local LaunchAgent so this schedule can wake the app when needed.")
