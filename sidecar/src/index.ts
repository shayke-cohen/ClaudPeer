...
const _debugLogPath = pathJoin(osHomedir(), ".claudestudio", "debug-ask-user.log");
appendFileSync(_debugLogPath, `[${new Date().toISOString()}] SIDECAR STARTING (modified code loaded)\n`);
console.log("[claudestudio-sidecar] Starting...");
...
console.log("[claudestudio-sidecar] Ready.");
...
process.on("SIGINT", () => {
  console.log("\n[claudestudio-sidecar] Shutting down...");
  sseManager.close();
  wsServer.close();
  httpServer.close();
  process.exit(0);
