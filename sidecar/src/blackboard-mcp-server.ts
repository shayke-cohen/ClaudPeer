#!/usr/bin/env bun
/**
 * Standalone MCP server exposing the Odyssey blackboard as tools.
 * Communicates via JSON-RPC over stdio (MCP protocol).
 * Connects to the blackboard HTTP API at ODYSSEY_HTTP_PORT (default 9850).
 *
  ...
  if (msg.method === "initialize") {
    sendResponse({
      ...
      result: {
        ...
        capabilities: { tools: {} },
        serverInfo: { name: "odyssey-blackboard", version: "0.1.0" },
      },
    });
    return;
  }
