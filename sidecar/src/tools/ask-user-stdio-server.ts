 ...
 * Connects to the sidecar HTTP API at ODYSSEY_HTTP_PORT (default 9850).
 *
 * Each interactive session spawns one instance of this server.
 * The sidecar passes the session ID via ODYSSEY_SESSION_ID env var.
 */

const HTTP_PORT = parseInt(process.env.ODYSSEY_HTTP_PORT ?? process.env.CLAUDESTUDIO_HTTP_PORT ?? "9850", 10);
const SESSION_ID = process.env.ODYSSEY_SESSION_ID ?? process.env.CLAUDESTUDIO_SESSION_ID ?? "unknown";
